...
void Enemy::Initialize(const Vector3& position, Audio* audio, UseScene useScene) {
	//使用されるシーン設定
	useScene_ = useScene;
	...
	case USETITLE:
		...
		// 腹部の初期化
		stomach_->Initialize(modelStomach_.get(), position, rad_,useScene_);
		// 胸部の初期化
		chest_->Initialize(modelChest_.get(), rad_, useScene_);
		// 頭部の初期化
		head_->Initialize(modelHead_.get(), rad_, useScene_);
		break;
	case USESTAGE:
		// オーディオ
		audio_ = audio;
		...
		soundHandleCharge_ = audio_->LoadWave("Audio/chargeEnemySE.wav");
		soundHandleMediumDamage_ = audio_->LoadWave("Audio/damageMediumSE.wav");
		soundHandleLargeDamage_ = audio_->LoadWave("Audio/damageLargeSE.wav");
		break;
	case USEDIRECTION:
		break;
	default:
